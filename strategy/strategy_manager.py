"""
StrategyManager: 根据 strategy_type 创建不同策略，从队列取 SnapshotProcessedV1，
...
from indicators import SnapshotProcessedV1
...
class StrategyManager:
    """
    根据 strategy_type 创建策略；从 input_queue 取 SnapshotProcessedV1，
    ...
    def __init__(
        ...
        self._trigger_timeframe = trigger_timeframe  # 若为 None，从首条 v1 取
    ...
    def _v1_to_context(self, v1: SnapshotProcessedV1, trigger_channel: str) -> StrategyContext:
        """从 SnapshotProcessedV1 及 V1 历史构建 StrategyContext。"""
        history_list: List[SnapshotProcessedV1] = list(self._snapshot_processed_v1_history)
        self._snapshot_processed_v1_history.append(v1)
        return StrategyContext(
            trigger_channel=trigger_channel,
            snapshot_processed_v1=v1,
    ...
    def _run_loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                v1 = self._input_queue.get(timeout=0.5)
            except Empty:
                continue
            if not isinstance(v1, SnapshotProcessedV1):
                continue
            if trigger_tf is None and v1.compose_snapshot:
                trigger_tf = next(reversed(v1.compose_snapshot))
                self._trigger_timeframe = trigger_tf
            try:
                context = self._v1_to_context(v1, trigger_tf)
                ...
                out["snapshot_ts"] = snap.ts if snap else v1.ts
