...
class CandlePctIndicator(BaseIndicator):
    ...
    def __init__(self, name: str, window: int = 5, max_buffer: int = 500, **kwargs: Any) -> None:
    ...
    def update(self, candle: CandleLike) -> None:
        ...
        if len(self._buffer) >= 2:
            pct = self._pct(self._buffer[-1], self._buffer[-2])
        abs_pct = round(abs(pct), 6) if pct is not None else None
